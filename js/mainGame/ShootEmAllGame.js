...
function ShootEmAllGame() {
	...
	this.init = function (playerName, missionLvl) {
		...
		if (!this.player) {
			...
			// socket.emit('player1 created', this.player)
		}

		// this.player2 = new Player();

		// socket.on('player2 created', function (player2) {
		// 	console.log('here at create 2', player2)
		// 	if (!this.player2 && player2) {
		// 	console.log('created 2', this.player2)

		// 	that.player2 = {...that.player2, ...player2};
		// 	console.log('mapped 2', this.player2)
		...
		if (!this.player2) {
			this.player2 = new Player();
			this.player2.height = 72;
			this.player2.width = 72;
			this.player2.x = WIDTH / 2 - that.player2.height - 100;
			this.player2.y = HEIGHT / 2 - that.player2.width;
	...
	that.updateFirstMissionEnemies = function (i) {
	...
	that.updateEnemiesAfterHostageFound = function (i) {
	...
	that.drawPlayer = function () {
		...
		socket.emit("player1 info", that.player)
		socket.on('player1 info', function (player2) {


			if (that.player2) that.player2 = { ...that.player2, ...player2 };

			// that.player2.draw(that.player2.playerRotation, keyState);

		});
		that.player.draw(that.player.playerRotation, keyState);
		that.player2.draw(that.player2.playerRotation);
	...
	that.drawFirstMissionUI = function () {
		...
		gameUI.writeText('Base Health Left: ' + base.health, 30, WIDTH / 2 - 150, 80); //parameters are text, fontsize, x, y, color
		gameUI.writeText('Time Left To Win: ' + survivalTimeLeft, 20, WIDTH / 2 - 90, 120);

		if (gameTime < 400) {
			gameUI.writeText('Mission 1: Survive and Protect your tank for ' + survivalTimeLeft, 25, 450, 180);
		}
	...
	that.drawSecondMissionUI = function () {
		...
		if (gameTime < 500) {
			gameUI.writeText('Mission 2: Find hostage and bring him back to helipad within ' + survivalTimeLeft, 25, 350, 180);
		}
